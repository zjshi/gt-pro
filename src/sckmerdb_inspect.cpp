...
void multi_btdc64(int n_path, char** kpaths) {	
	...
	*/
    ...
    for (auto ip = kdb.begin(); ip != kdb.end(); ip=ip+2) {
		/*
		int_type kmer_int = *ip;

		uint32_t lmer_int = 0;
		uint32_t mmer_int = 0;

		lmer_int = (uint32_t)((kmer_int & 0xFFFFFFFF00000000LL) >> 32);
		mmer_int = (uint32_t)(kmer_int & 0xFFFFFFFFLL);

        seq_decode<uint32_t>(lmer_buf, l+1, lmer_int, code_dict_32, b_mask_32);    
        seq_decode<uint32_t>(mmer_buf, m+1, mmer_int, code_dict_32, b_mask_32);    

		snp = *(ip+1); 
    	fh << lmer_buf << mmer_buf << '\t' << snp << '\n';
		*/
    ...
    // fh.write((char*)&kdb[0], kdb.size() * sizeof(int_type));
